...
(function ($) {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function updateMusicDetails(date) {
            ...
            var musicDetails = document.getElementById("music-details");

            if (musicInfo) {
                musicDetails.innerHTML = `
            <h3>${musicInfo.title}</h3>
            <button class="play-button" data-source="${musicInfo.source}">Play</button>
        `;
                // Add an event listener to the play button
                var playButton = musicDetails.querySelector('.play-button');
                playButton.addEventListener('click', function () {
                    // Post a message to musicPlayer.html with the music source
                    window.parent.postMessage({ type: 'changeMusic', source: musicInfo.source }, '*');
                });
        ...
        calendarDays.forEach(function (day) {
            day.addEventListener('click', function () {
                ...
                var clickedDate = year + '-' + (month + 1).toString().padStart(2, '0') + '-' + this.innerHTML;
                updateMusicDetails(clickedDate);
            });
        });
